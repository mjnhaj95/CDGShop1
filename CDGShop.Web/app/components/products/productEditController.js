    ...
    function productEditController(apiService, $scope, notificationService, $state, commonService, $stateParams) {
        ...
        function loadProductDetail() {
            apiService.get('api/product/getbyid/' + $stateParams.id, null, function (result) {
                ...
                $scope.moreImages = JSON.parse($scope.product.MoreImages); // Chuyển sang mảng
            }, function (error) {
        ...
        $scope.ChooseMoreImage = function () {
            var finder = new CKFinder();
            finder.selectActionFunction = function (fileUrl) {
                $scope.$apply(function () {                 //  $scope.$apply giúp tăng tốc độ load ảnh
...
})(angular.module('cdgshop.products'));